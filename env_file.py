...
class UR5eEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.action_space = spaces.Box(
            low=-0.1, high=0.1, shape=(7,), dtype=np.float64
        )
        self.target_position = np.array([0.5, 0, 1.3])
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        with self._physics.reset_context():
            # Set stable joint configuration
            self._physics.bind(self._arm.joints).qpos = [
                0.0, -1.5707, 1.5707, -1.5707, -1.5707, 0.0
            ]
            # Set initial target in front of the robot to encourage reaching
    ...
    def step(self, action: np.ndarray) -> tuple:
        pose = self._target.get_mocap_pose(self._physics)
        current_position, current_orientation = pose[0:3], pose[3:]
        target_position = current_position + action[:3]
        target_orientation = current_orientation + action[3:]
        ...
        self._controller.run(np.concatenate((target_position, target_orientation)))
        print("Gravity:", self._physics.model.opt.gravity)

        # Step the physics simulation
        self._physics.step()

        # Optional: Render frame only if necessary
        ...
        distance_to_target = np.linalg.norm(eef_position - self.target_position)
        reward = -distance_to_target*1000
        print("distance", distance_to_target)
        terminated = bool(distance_to_target < 0.05)  # Cast explicitly to boolean

        # Additional environment info
        info = self._get_info()

        # Return four elements for compatibility with gym
